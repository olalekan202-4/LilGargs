...
const NavLink = ({ href, children, isActive, onClick }) => (
    ...
    >
        ...
        <span className={`absolute bottom-[-4px] left-0 w-full h-0.5 bg-purple-500 transition-transform origin-center duration-300 ${isActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`}></span>
    </a>
);
    ...
    return (
        <>
            <motion.header 
                ...
                className={`sticky top-0 z-40 transition-all duration-300 ${isScrolled ? 'bg-gray-900/90 backdrop-blur-lg shadow-2xl' : 'bg-gray-900/50 backdrop-blur-md'}`}
            >
                <div className="container mx-auto flex justify-between items-center p-4 text-white">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button className="md:hidden z-50" onClick={() => setIsMenuOpen(!isMenuOpen)}>
                            <motion.div animate={isMenuOpen ? "open" : "closed"} className="w-6 h-5 flex flex-col justify-between items-center">
                                <motion.span variants={{ closed: { rotate: 0, y: 0 }, open: { rotate: 45, y: 8 } }} className="block w-full h-0.5 bg-white"></motion.span>
                                <motion.span variants={{ closed: { opacity: 1 }, open: { opacity: 0 } }} className="block w-full h-0.5 bg-white"></motion.span>
                                <motion.span variants={{ closed: { rotate: 0, y: 0 }, open: { rotate: -45, y: -8 } }} className="block w-full h-0.5 bg-white"></motion.span>
                            </motion.div>
                        </button>
                    </div>
