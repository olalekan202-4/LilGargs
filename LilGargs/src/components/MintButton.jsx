...
const MintButton = ({ showMessage, refresh, launchpadInfo }) => {
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      showMessage(
        `Mint successful! Transaction: ${txid.substring(0, 10)}...`,
        "success"
      );
      ...
      showMessage(
        error.message || "An unknown error occurred during mint.",
        "error"
      );
    } finally {
      setIsMinting(false);
  ...
  const isMintDisabled =
    isMinting || !launchpadInfo || !launchpadInfo.publicPhaseStart;
  const mintButtonText = !launchpadInfo?.publicPhaseStart
    ? "Mint Not Started"
    : "Mint Now";
    ...
    <div className="flex flex-col items-center gap-6 p-8 bg-gray-800/50 rounded-2xl shadow-xl border border-gray-700 h-full">
      ...
      <div className="flex items-center gap-4">
        <motion.button
          ...
          disabled={isMinting}
          className="px-5 py-2 bg-gray-700 rounded-lg text-2xl font-bold hover:bg-gray-600 transition disabled:opacity-50"
        >
          -
        </motion.button>
        <span className="text-4xl font-bold text-purple-400 w-16 text-center">
          {mintCount}
        </span>
        <motion.button
          whileTap={{ scale: 0.9 }}
