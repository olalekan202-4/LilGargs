...
const TierProgress = ({ ownedNfts }) => {
    ...
    const currentTierIndex = tiers.slice().reverse().findIndex(tier => ogCount >= tier.minOgs);
    const currentTier = tiers[tiers.length - 1 - currentTierIndex];
    const nextTier = tiers[tiers.length - currentTierIndex];
    
    const progressToNextTier = nextTier ? ((ogCount - currentTier.minOgs) / (nextTier.minOgs - currentTier.minOgs)) * 100 : 100;
    ...
    return (
        ...
        >
            <h2 className="text-3xl font-bold text-center text-cyan-300 mb-6">Miner Tier</h2>
            <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-2 text-white">
                    <span className="font-bold text-lg">{currentTier.name}</span>
                    {nextTier && <span className="text-sm text-gray-400">Next Tier: {nextTier.name} at {nextTier.minOgs} OGs</span>}
                </div>
                ...
                <p className="text-center text-xs text-gray-500 mt-3">Higher tiers unlock future boosts, bonuses, and bragging rights in the Gargiverse.</p>
            </div>
        </motion.section>
    );
};
